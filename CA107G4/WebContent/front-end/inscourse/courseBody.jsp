...
<jsp:useBean id="inscCourseTimeSvc" scope="page"
	class="com.inscoursetime.model.InsCourseTimeService" />
<jsp:useBean id="courseReservationSvc" scope="page"
	class="com.coursereservation.model.CourseReservationService" />
...
<jsp:useBean id="courseSvc" scope="page"
	...
	class="com.coursetype.model.CourseTypeService" />
...
<link
	href="https://cdn.bootcss.com/limonte-sweetalert2/7.33.1/sweetalert2.css"
	rel="stylesheet">
<script
...
.star-vote>.add-star {
	background-image:
		url("<%=request.getContextPath()%>/images/inscourse/yellow-star.png");
}

.star-vote>.del-star {
	background-image:
		url("<%=request.getContextPath()%>/images/inscourse/white-star.png");
	...
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="#">課程類型</a></li>
			 <fmt:parseNumber var = "i" type = "number" value = "${courseSvc.findByLike(param.courseName).courseTypeId}" />	
			<li class="breadcrumb-item"><a href="#">${courseTypeSvc.findOneById(i).courseTypeName}</a></li>
...
Swal.fire(
	...
	<form id="a"
		action="<%=request.getContextPath()%>/InsCourseServlet" method="POST">
				...
				<div class="row">
					<div class="col-lg-8 col-md-8 col-sm-7 col-xs-12">
						<div class="box d-flex" id="tc1">
							<div class="tcimg">
								<a
									href="<%=request.getContextPath()%>/member/viewAsMember.jsp?memId=${param.memId}">
									<img
									src="<%=request.getContextPath()%>/member/DBGifReader.do?memId=${param.memId}"
							...
							<div class="teachName">
								...
								<div id="star_con" class="star-vote">
									<span id="add_star" class="add-star"></span> <span
							...
							<div class="accordion" id="heading">
								...
								<div class="card">
									<div class="card-header">
										<h2 class="mb-0">
											<button class="btn btn-link" type="button"
												...
												aria-expanded="true" aria-controls="collapseOne" value="">
												${param.courseName}</button>
										</h2>
									</div>
									<div class="collapse show" aria-labelledby="headingOne"
										...
										<div class="card-body">
											<p>
												<span class="badge badge-pill badge-info">類型</span>
              		...
              		團體課程
											...
											<p />
											<p>
												<span class="badge badge-pill badge-success">地點</span>${param.inscLoc}
											<p />
											<p>
												<span class="badge badge-pill badge-primary">授課語言</span>${param.inscLang}
											<p />
											<p>
												<span class="badge badge-pill badge-warning">價錢</span>$${param.inscPrice}元(每小時)
											...
											<p />
											<p>
												<span class="badge badge-pill badge-secondary">課程大綱</span>${param.inscCourser}
											<p />

											<input type="hidden" name="inscType"
												value="${param.inscType}"> <input type="hidden"
												name="inscLoc" value="${param.inscLoc}"> <input
												type="hidden" name="inscLang" value="${param.inscLang}">
											<input type="hidden" name="inscPrice"
												value="${param.inscPrice}"> <input type="hidden"
												name="inscCourser" value="${param.inscCourser}">
						...
						<div class="box">
							...
							<input type="hidden" name="teacherText"
								value="${param.teacherText}">
							<h3 class="box-title">學歷</h3>
							<p>${param.teacherEdu}</p>
							<input type="hidden" name="teacherEdu"
								value="${param.teacherEdu}">
						...
						<div class="box">
							...
							<div class="container">
								<div class="row text-center">
									<c:forEach var="courseReservationVO" end="2"
										items="${courseReservationSvc.findByRate(param.inscId)}">
										<div class="col-sm-6 col-md-4">
											<div class="testimonial-box">
												<img
													src="<%=request.getContextPath()%>/member/DBGifReader.do?memId=${courseReservationVO.memId}"
													width="120" height="120" alt="" />
							...
							<div class="row">
								<div class="col-12">
									<div class="datePicker">
										<span class="badge badge-primary">請選擇日期</span> <input
											type="text" class="form-control" name="inputSuccess"
											id="inputSuccess">
										<!-- f_date1見第30行  -->
	...
	<form id="b"
		...
		<div class="row">
			<c:forEach var="inscCourseTimeVO" items="${list}">
				...
				<div class="col-1"></div>
				<div class="col-5">
					<div class="form-check" onclick="grab('${inscCourseTimeVO.inscTimeId}');">
						<input class="form-check-input" type="radio" name="inscDate"
							id="${inscCourseTimeVO.inscTimeId}"
							value="${inscCourseTimeVO.inscTimeId}" /> <label
							class="form-check-label" for="exampleRadios1"> <fmt:formatDate
								value="${inscCourseTimeVO.inscMFD}" pattern="HH:mm" /> <fmt:formatDate
								value="${inscCourseTimeVO.inscEXP}" pattern="HH:mm" />
						</label>
		...
		<input type="hidden" name="memId" value="${param.memId}"> <input
			type="hidden" name="inscType" value="${param.inscType}"> <input
			type="hidden" name="inscLoc" value="${param.inscLoc}"> <input
			type="hidden" name="inscLang" value="${param.inscLang}"> <input
			type="hidden" name="inscPrice" id="inscPrice"
			value="${param.inscPrice}"> <input type="hidden"
			name="inscCourser" value="${param.inscCourser}"> <input
			type="hidden" name="teacherText" value="${param.teacherText}">
	...
	<form id="c" action="<%=request.getContextPath()%>/CourseReservationServlet" method="POST">
		<input type="hidden" name="action" value="addOrder"/> 
		<input type="hidden" name="memName" value="${param.memName}"/> 
		<input type="hidden" name="teacherId" value="${param.teacherId}"/>
		<input  type="hidden"name="inscId" value="${param.inscId}"/> 
		<input type="hidden" name="crvLoc" value="${param.inscLoc}"/>
		<input  type="hidden"id="crvTotalTime" name="crvTotalTime"/>
		<input  type="hidden" id="crvTotalPrice" name="crvTotalPrice"/>
		
		<input type="hidden" id="crvMFD" name="crvMFD"/> 
		<input type="hidden" id="crvEXP" name="crvEXP"/>
		<input type="hidden" id="memId" name="memId"/>
		<input  type="hidden" id="inscTimeId2" name="inscTimeId"/>
			...
			<div class="summary-block">
				...
				<div class="summary-block">
					<div class="summary-content">
						...
						<div class="summary-price">
							<p class="summary-text" id="price">
								<!-- 								$ -->
								<%-- 								<c:out value="${crvPrice}" default="0" /> --%>
							</p>
							
							<span class="summary-small-text pull-right">元(總計小時)</span>
						</div>
					</div>
				</div>
				<div class="summary-block">
					<div class="summary-content">
						...
						<div class="summary-price">
							<p class="summary-text" id="tax">
								<!-- 								$ -->
								<%-- 								<c:out value="${crvTax}" default="0" /> --%>
							</p>
							<span class="summary-small-text pull-right">手續費(10%)</span>
						</div>
					</div>
				</div>
				<div class="summary-block">
					<div class="summary-content">
						...
						<div class="summary-price">
							<p class="summary-text" id="totalPrice">
								<!-- 								$ -->
								<%-- 								<c:out value="${crvTotalPrice}" default="0" /> --%>
								
							</p>
							<span class="summary-small-text pull-right">元</span>
						</div>
					</div>
				</div>
				<br>
				...
				<div class="d-flex justify-content-center" id="orderSumit">
					<input type="button" id="readyGo" class="btn btn-success"
						value="送出預定 ">
				</div>
	...
	<script>
		...
		function connect() {
			...
			webSocket.onmessage = function(event) {
				...
				var form_checks = document.getElementsByClassName("form-check");

				var inscTimeId = event.data;
				
				
				$.each(form_checks, function(i) {
					if ($(this).children(".form-check-input").val() == inscTimeId) {
		...
		function grab(inscTimeId) {
			
			webSocket.send(inscTimeId);
			
			
		 	var inscPrice = $('#inscPrice').val();
		 	 ...
		 	 $.ajax({
				...
				success: function(obj) {
					$('#time').text(obj.time);
					$('#price').text("$"+obj.crvPrice);
					$('#tax').text("$"+obj.crvTax);
					$('#totalPrice').text("$"+obj.crvTotalPrice);
					$('#crvTotalTime').val(obj.crvTotalTime);
					$('#crvTotalPrice').val(obj.crvTotalPrice);
					
					$('#inscTimeId2').val(obj.inscTimeId);
					//$('#memId').val(obj.memId);
					$('#crvMFD').val(obj.inscMFD);
					$('#crvEXP').val(obj.inscEXP);
