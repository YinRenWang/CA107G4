<%@ page contentType="text/html; charset=UTF-8" pageEncoding="Big5"%>
<%@ page import="com.goods.model.*"%>
<%-- 此頁暫練習採用 Script 的寫法取值 --%>

<%
  GoodsVO goodVO = (GoodsVO) request.getAttribute("goodVO"); //EmpServlet.java(Concroller), 存入req的empVO物件
%>

<html>
<head>
<title>員工資料 - listMemEmp.jsp</title>

<style>
  table#table-1 {
	background-color: #CCCCFF;
    border: 2px solid black;
    text-align: center;
  }
  table#table-1 h4 {
    color: red;
    display: block;
    margin-bottom: 1px;
  }
  h4 {
    color: blue;
    display: inline;
  }
</style>

<style>
  table {
	width: 600px;
	background-color: white;
	margin-top: 5px;
	margin-bottom: 5px;
  }
  table, th, td {
    border: 1px solid #CCCCFF;
  }
  th, td {
    padding: 5px;
    text-align: center;
  }
</style>

</head>
<body bgcolor='white'>

<h4>此頁暫練習採用 Script 的寫法取值:</h4>
<br><%= goodVO==null %><---[false代表有拿到goodVO物件--true則沒有]
<table id="table-1">
	<tr><td>
		 <h3>員工資料 - ListOneGood.jsp</h3>
		 <h4><a href="select_page.jsp"><img src="images/back1.gif" width="100" height="32" border="0">回首頁</a></h4>
	</td></tr>
</table>

<table>
	<tr>
		<th>商品編號</th>
		<th>教師編號</th>
		<th>書名</th>
		<th>價格</th>
		<th>商品資訊</th>
		<th>商品圖片</th>
		<th>商品狀態</th>
	</tr>
	<tr>
		<td><%=goodVO.getGoodId()%></td>
		<td><%=goodVO.getTeacherId()%></td>
		<td><%=goodVO.getGoodName()%></td>
		<td><%=goodVO.getGoodPrice()%></td>
		<td><%=goodVO.getGoodInfo()%></td>
		<td><img src="<%= request.getContextPath()%>/goods/DBGifReaderGoods.do?goodId=${goodVO.goodId}"></td>
		<td><%=goodVO.getGoodStatus()%></td>
	</tr>
</table>

</body>
</html>